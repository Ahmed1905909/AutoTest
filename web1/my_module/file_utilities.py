...
# Generates and returns the pytest code as a string
def generate_pytest_code(metadata, test_suite):
    code = "import " + metadata["file"] + "\nimport pytest\n"
...
# Returns the generated pytest code as an HTTP response
def generate_pytest_response(request):
    # Retrieve metadata and test suite from request
    metadata = request.POST.get('metadata')
    test_suite = json.loads(request.POST.get('test_suite'))

    # Generate the pytest code
    pytest_code = generate_pytest_code(metadata, test_suite)

    # Return the pytest code as an HTTP response
    response = HttpResponse(pytest_code, content_type='text/plain')

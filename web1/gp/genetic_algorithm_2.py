...
import getopt
import sys
from gp.generation_utilities import *
from gp.fitness_functions import *
...
def add_random_action(test_suite,metadata):
    suite_size = len(test_suite) - 1
...
def change_random_parameter(test_suite,metadata):
...
def add_test_case(test_suite,metadata):
...
def mutate(solution,metadata):
    ...
    if action == 1:  # delete an action
        ...
        new_solution.test_suite = add_random_action(suite,metadata)
    elif action == 3:  # change random parameter
        new_solution.test_suite = change_random_parameter(suite,metadata)
    elif action == 4:  # add a test case
        new_solution.test_suite = add_test_case(suite,metadata)
...
def create_population(size,metadata):
    pop = []
...
def crossover(parent1, parent2,metadata):
...
def uniform_crossover(parent1, parent2,metadata):
...
metadata_location = "example/Metadata.json"
...
def generate_best_test_suite(metadata):

# Create initial population.
    population = create_population(population_size,metadata)
        ...
        while len(new_population) < len(population):
            ...
            if random.random() < crossover_probability:
                if crossover_operator == "single":
                    (offspring1, offspring2) = crossover(offspring1, offspring2,metadata)
                else:
            ...
            if random.random() < mutation_probability:
                offspring1 = mutate(offspring1,metadata)
            if random.random() < mutation_probability:
                offspring2 = mutate(offspring2,metadata)
...
# Print the best test suite to a file
    outfile = write_to_file_2(metadata, solution_best.test_suite)
    return outfile
